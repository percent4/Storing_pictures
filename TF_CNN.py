...
class CNN:
    ...
    def __init__(self, epoch, learning_rate, save_model_path):
        ...
        """
        第一层 卷积层和池化层
        x_image(batch, 16, 16, 1) -> h_pool1(batch, 8, 8, 10)
        """
        x = tf.placeholder(tf.float32, [None, 256])
        self.x = x
        x_image = tf.reshape(x, [-1, 16, 16, 1])  # 最后一维代表通道数目，如果是rgb则为3
        ...
        """
        第二层 卷积层和池化层
        h_pool1(batch, 8, 8, 10) -> h_pool2(batch, 4, 4, 20)
        """
        ...
        """
        第三层 全连接层
        h_pool2(batch, 4, 4, 20) -> h_fc1(1, 100)
        """
        W_fc1 = self.weight_variable([4 * 4 * 20, 100])
        ...
        第五层 Softmax输出层
        """
        W_fc2 = self.weight_variable([100, 10])
        ...
        """
        self.y_true = tf.placeholder(shape = [None, 10], dtype=tf.float32)
